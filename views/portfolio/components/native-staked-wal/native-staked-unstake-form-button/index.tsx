...
import { useNativeStakedWalUnstakeAction } from './native-staked-unstake-form-button.hooks';
import NativeStakedWalUnstakeFormButtonPreview from './native-staked-unstake-form-button-preview';

const NativeStakedWalUnstakeFormButton: FC = () => {
  const { coins } = useCoins();
  const { setContent } = useModal();
  const form = useFormContext();
  const { onUnstake, loading } = useNativeStakedWalUnstakeAction();
  ...
  const handleUnstake = () =>
    setContent(
      <FormProvider {...form}>
        <NativeStakedWalUnstakeFormButtonPreview onProceed={onUnstake} />
      </FormProvider>,
      {
...
export default NativeStakedWalUnstakeFormButton;
